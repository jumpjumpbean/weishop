...
class FollowModel extends Model {
	...
	function init_follow($openid, $token = '', $has_subscribe = false) {
		...
		addWeixinLOg($config,'UserCenter-addonConfig');	
		$user = array (
		...
		addWeixinLog($user,'newuser');
		$data ['uid'] = $uid = D ( 'Common/User' )->add ( $user );
		addWeixinLog('newuser-uid',$uid);	
		if ($has_subscribe !== false) {
			$data ['has_subscribe'] = $has_subscribe;
		}
		M ( 'public_follow' )->add ( $data );
		$data2 ['nickname']= $user2['nickname'];
		$data2 ['headimage']=$user2['headimgurl'];
		$data2 ['name']=$user2['truename'];
		$data2 ['phone']=$user2['mobile'];
		$data2 ['token']=$data ['token'];
		$data2 ['openid']=$data ['openid'];	
		$data2 ['uid']=$uid;	
		$data2 ['subscribe_time']=$user2['subscribe_time'];
		$data2 ['subscribe_status']=1;
		
		M ( 'hxtx_user' )->add ( $data2 );
