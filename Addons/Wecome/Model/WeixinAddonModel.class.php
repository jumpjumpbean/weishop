...
class WeixinAddonModel extends WeixinModel {
	function reply($dataArr, $keywordArr = array()) {
		...
		if ($dataArr ['Content'] == 'subscribe') {
			...
			// 增加积分
			session ( 'mid', $uid );
			addWeixinLog('###---subscribe-newuser-mid-',$uid);
            addWeixinLog('###---subscribe-newuser-session-mid-',session('mid'));
            addWeixinLog('###---subscribe-newuser-session-id-',session_id());
...
// 					$res = $this->replyNews ( $articles );
            ...
            addWeixinLog('###---subscribe-end-session-mid-',session('mid'));
		} elseif ($dataArr ['Content'] == 'scan') {
			$this->scan ( $dataArr, $keywordArr, $config );
			addWeixinLog('###---scan',$dataArr);
		} elseif ($dataArr ['Content'] == 'unsubscribe') {
			// 直接删除用户
			addWeixinLog('###---unsubscribe',$dataArr);
			...
			$map3 ['openid'] = $dataArr ['FromUserName'];
			$map3 ['token'] = get_token ();
			$map3 ['uid'] = D ( 'public_follow' )->where ( $map1 )->getField ( 'uid' );
			M ( 'hxtx_user' )->where ( $map3 )->setField ( 'subscribe_status', 0 );
			$untime=time();
			M ( 'hxtx_user' )->where ( $map3 )->setField ( 'unsubscribe_time', $untime );
			// 增加积分
