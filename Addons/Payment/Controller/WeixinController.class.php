...
use Addons\Pjt\Controller\PjtApiService;
...
//                 $orderDao->where($map)->setField('pay_status', $isPay);
                ...
                $status = 1;
                if ($orderInfo['is_pjt']) {
                    //addWeixinLog('payment_payok:orderInfo', $orderInfo);
                    $pjtController = A('Addons://Pjt/Pjt');
                    $pjtController->createOrder($orderInfo);
                    // 票景通商品支付后状态为配送中
                    $status = 3;
                    // 票景通商品支付后发货状态设为已发货
                    $save ['is_send'] = 2;
                ...
                $res = $orderDao->update ( $orderid, $save );
                $orderDao->setStatusCode ( $orderid, $status);
